...
const Navbar = () => {
  ...
  return (
    <Box  bg = {HEADER_BG}>
          ...
          <Flex fontSize={NAVBAR_MENU_HEADING} gap={'20px'} display={['none','none','none','flex','flex']} justifyContent={'space-between'} alignItems={'center'} >
            <Flex  gap={'20px'} alignItems={'center'}>
              ...
              <Menu >
                ...
                <Link href="" ><Text 
                color={ ourServicesPath.includes(pathname) ? TEXT_HOVER : TEXT_COLOR} fontSize={NAVBAR_MENU_HEADING}>Our Services</Text></Link>
                </MenuButton>
                <MenuList  zIndex={'2'}>
                  <MenuItem>
      ...
      {isOpen ? (
        <>
          <Box mx={SECTION_MARGIN_Y} pb={'20px'} display={{ md: 'none' }}>
            <Flex gap={'20px'} flexDir={'column'} color={TEXT_COLOR} fontSize={TEXT_FONT_SIZE}>
              ...
              <Menu >
                <MenuButton  onClick={()=>{router.replace('/our_services')}} p={0} bg={HEADER_BG} _active={{bg:HEADER_BG}} _hover={{bg:HEADER_BG}} >
                <Link href="/our_services" ><Text  color={ ourServicesPath.includes(pathname) ? TEXT_HOVER : TEXT_COLOR} textAlign={'left'} fontSize={['14px','16px']} fontWeight={'400'}>Services</Text></Link>
                </MenuButton>
